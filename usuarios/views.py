...
from bibliotecaTeste.models import Receita
...
def cadastro(request):
    if request.method == 'POST':
        ...
        if campo_vazio(nome):
            messages.error(request,'Insira o Nome Completo.')
            return redirect('cadastro')
        if campo_vazio(email):
            messages.error(request,'O E-mail não pode ficar em branco!')
            return redirect('cadastro')
        if senha_nao_identificas(senha,senha2):
            messages.error(request, 'As senhas não está idênticas.')
            return redirect('cadastro')
...
def login(request):
    if request.method == 'POST':
        ...
        if campo_vazio(email) or campo_vazio(senha):
            messages.error(request,'Insera E-mail e a Senha!')
            return redirect('login')
        if User.objects.filter(email=email).exists():
...
def senha_nao_identificas(senha,senha2):
    return senha != senha2
